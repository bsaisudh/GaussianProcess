...
yTest = np.sin(np.multiply(3,xTest))
...
for ndx, kernel in zip([1,2,3,4], [kernel1, kernel2, kernel3, kernel4]):
    ...
    GP.append(gp)
    with open( f"Data/GP_for_Kernel_Sin3x_{ndx}.pkl", "wb" ) as f:
        pickle.dump(GP, f)
    print(f'GP for Kernel - {ndx} Finished :: Elapsed Time - {time.time()-t}')
    print(datetime.datetime.now())
    print('---------------------------------------------------------------------------')
    
modelFileName = "Data/GP_for_Kernel_Sin3x_4.pkl"
...
for ndx, gp, ax in zip([1,2,3,4], GP, [[0,0],[0,1],[1,0],[1,1]]):
    ...
    axs[ax[0],ax[1]].fill(np.concatenate([xTest[:,0], xTest[::-1,0]]),
             np.concatenate([y_pred - 2 * sigma,
                            (y_pred + 2 * sigma)[::-1]]),
             alpha=.5, fc='b', ec='None', label='100% confidence interval')
